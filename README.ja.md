...
`PUPPETEER_EXECUTABLE_PATH` で実行ファイルを明示するか、Linux と Windows の標準的な配置場所にある
Chromium / Google Chrome を自動検出して使用します（Windows では Chromium ベースの Microsoft Edge も
フォールバックとして使います）。開発用 Docker イメージには Chromium が含まれます。

標準的でない場所にインストールしている場合や、自動検出の候補を持たない macOS などでは、
`PUPPETEER_EXECUTABLE_PATH` で実行ファイルを明示してください。SEA 単体バイナリの
...
html:
  ...
  branding: true # false で「Generated by monodocs」フッターを非表示
```
