...
脆弱性や機密情報に関わる問題を、公開 Issue、Pull/Merge Request、Discussion へ投稿しないでください。

GitHub への移行が完了するまでは、`***REMOVED***` 宛てにメールで報告してください。
件名の先頭に `[monodocs security]` を付け、本文や添付ファイルへ必要以上の秘密情報を含めないで
ください。GitLab プロジェクトメンバーで confidential issue を作成できる場合は、その経路も
利用できます。GitHub への移行後は、この文書を更新し、GitHub の非公開脆弱性報告経路を正本とします。
