...
/**
 * 種別アイコンの SVG パス（GitHub Primer octicons。16x16 viewBox）。
 * CSS の `mask-image` ではなく **インライン SVG** として本文へ埋め込む。mask を使うと
 * PDF（`page.pdf()`）出力時にソフトマスク（SMask）としてエンコードされ、一部の PDF
 * ビューアがマスクを描けず塗り色の四角に見える。ベクタのインライン SVG なら全ビューアで
 * 正しく描画される。色は CSS 側で `fill: currentColor`（種別色）に委ねる。
 */
const ADMONITION_ICON_PATH: Record<AdmonitionType, string> = {
...
/** 種別アイコンをインライン SVG の raw ノードとして返す（`fill` は CSS の currentColor）。 */
function admonitionIconNode(type: AdmonitionType): ElementContent {
  return {
    type: "raw",
    value:
      `<svg class="admonition-icon" viewBox="0 0 16 16" width="16" height="16" aria-hidden="true">` +
      `<path fill="currentColor" d="${ADMONITION_ICON_PATH[type]}"></path></svg>`,
    ...
    children: [
      {
        ...
        children: [admonitionIconNode(type), { type: "text", value: ADMONITION_LABEL[type] }],
      },
      {
        type: "element",
        tagName: "div",
...
async function processMermaidPrerender(
  ...
  for (const block of blocks) {
    try {
      ...
      // （図単位のフォールバックで握りつぶさない。fail fast）。MermaidPrerenderSetupError も
      // 含めた BrowserSetupError 全般を対象にする。
