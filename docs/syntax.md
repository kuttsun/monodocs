...
- フェンスドコードブロック（``` ```lang ```）。`language-xxx` クラスを付与する
...
- ソースブロック（`[source,lang]`）、リテラル / リスティング / 例 / サイドバー / 引用 / 詩ブロック
...
| コードハイライト（shiki） | **未対応**（後続予定） | コードブロックには `language-xxx` クラスのみ付与。実ハイライトは後続バージョンで対応予定 |
