...
| Page break (`<div class="page-break"></div>` in Markdown, `<<<` in AsciiDoc)                                                  | **Supported** (v0.11)             | Starts a new sheet in the PDF, and when the HTML is printed from a browser. In Markdown, `<div style="page-break-after: always"></div>` is accepted as the same marker; both are replaced with an element monodocs builds, so no attribute of the input survives. The marker must be a block of its own — one inside a blockquote, a list item, a table cell, or a heading is not one, because the print stylesheet keeps those blocks together |
