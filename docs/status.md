...
- [x] Run [`scripts/verify-windows-binary.ps1`](../scripts/verify-windows-binary.ps1) against the published `v0.12.0-beta.1` assets on a Windows 11 host: all sixteen checks pass. The host sits behind a corporate proxy, which answered for `127.0.0.1` and turned the two `serve` checks into 503s while the binary was fine; #122 sends the loopback checks direct, and the rerun passed sixteen of sixteen
