...
- [x] `sources.asciidoc.attributes` sets presentational attributes such as `sectnums` and an author's own attributes, as **defaults** rather than locked values, so a document that sets its own wins — the opposite of Asciidoctor's API behaviour and what a configuration file should mean. Every value carries the trailing `@` that is Asciidoctor's own soft-set marker, measured in #110
- [x] `allow-uri-read`, `docinfo`, `backend`, `data-uri`, `imagesdir`, `source-highlighter`, and `sd-*` are refused, naming the attribute and the reason. `safe` and `base_dir` are not accepted at all, because a sandbox a configuration file can widen is a sandbox in name — and with them `docdir`, `docfile`, `docname`, `docfilesuffix`, `outdir`, and `showtitle`, which either decide where a path resolves or are what monodocs builds the page title and the element IDs from. Unsetting is not offered, and a value already ending in `@` is refused
- [x] An `include::` or an image whose real path resolves outside the input root is refused, naming the path it resolved to. A test uses an actual symbolic link, since Asciidoctor's safe mode does not resolve them — a linked file and a linked directory both pulled outside content into the output before this. The include check is a static one over the source text, so a target built from an attribute reference is skipped rather than guessed at; the reason an `IncludeProcessor` is not used is measured and recorded ([roadmap.md](roadmap.md) 17.5)
