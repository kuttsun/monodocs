...
- `themes/default/layout.test.ts` … sidebar reachability (the search box stays in place while the navigation tree scrolls to its end, and a viewport too short for the column falls back to scrolling the whole sidebar rather than clipping the tree). Reachability is decided by hit-testing rather than by element rectangles, which ignore clipping by an ancestor. This is CSS layout, so happy-dom cannot see it and the file is confirmed only in environments with a real Chromium
