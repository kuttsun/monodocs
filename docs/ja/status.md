...
0.11.0 はリリース済みである。改ページと 1.0 の契約という 2 つの半分は、2 回に分けず 1 つのリリース
として出した。理由は [roadmap.md](roadmap.md) の v0.11 に記録している。
...
- [ ] [syntax.md](syntax.md) が、既定がどちらの規則かを述べる——今は「改行」としか書いていない——とともに、Markdown が持つ明示的な改行 2 つ、行末スペース 2 つとバックスラッシュを書き、前者は行末空白を削るエディタでは残らないことを添える。サイトの設定リファレンスとその日本語ミラーがキーを載せる
