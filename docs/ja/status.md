...
- [x] `pdf.density` がプリセット名（`normal` / `compact` / `tight`）かオブジェクトを取り、枚数を決める 4 つの値（基準文字サイズ・行送り・見出し上の空き・表のセル余白）を動かす。見出し・段落・箇条書きと 24 行の表からなる日本語の事業書類で、3 つのプリセットはそれぞれ 5 枚・3 枚・2 枚になる。それまで唯一の手段だった `pdf.margin` は、同じ文書を実用範囲の全域で 9 枚のままに置いていた
...
- [x] `normal` は規則を 1 つも出力しない。密度を指定しない文書は以前と 1 バイトも変わらずに印刷される（読者がブラウザから印刷するときに、その人自身の基準文字サイズを継承することも含めて）。書き出されるのは `normal` と異なる値だけ
