...
- [ ] すべてのエラーと警告が安定した `code` を持ち、パイプラインが知っている場合は `path` と位置を持つ。`formatSourceRef` は既にファイルと位置を散文に組み立てており、位置は存在していて出口で潰されている
- [ ] コード無しで診断を足したらテストが落ちる
- [ ] メッセージカタログとコード集合は別物のままにする。メッセージキーは文言を選び、コードは所見を識別する。2 つのメッセージが 1 つのコードを共有してよく、コードを持たないメッセージがあってよい

**`validate`**（[roadmap.md](roadmap.md) 25.5）

- [ ] `monodocs validate --format json` が、スキーマバージョンと診断の配列を持つオブジェクトを出す。人間向けの出力は変わらない。`--strict` は追加しない——警告だけでも既に非ゼロで終了する
- [ ] 見出しレベルの飛び（`h2` の次が `h4`）を報告する
- [ ] `alt` 属性の無い画像を報告し、明示的な空の `alt=""` は報告しない。後半をテストが主張する。それが装飾画像を示す書き方だからである
- [ ] 既にビルド時に警告している、解決できない別ファイルのアンカーが、コード付きの診断として現れる
- [ ] 外部リンクは検査せず、孤立ページも報告しない。それぞれ理由を記録する（[roadmap.md](roadmap.md) 25.5）
...
- [ ] `root: .` と `sources.include: ["README.md", "docs/**"]` が、リポジトリが実際に取っている形のまま 1 つの文書を組み、画像・リンク・`monodocs.config.yml` を `root` から解決する
- [ ] `sources.exclude` は最後に差し引く。下書きを除くパターンが、それを含む include に打ち消されない
- [ ] どちらのキーも無い設定は今日とまったく同じに振る舞い、既存の fixture をそのままビルドするテストがそれを示す
- [ ] `input` は改名も非推奨化もしない。`root` の外を指す場合はマージではなくエラーになる
- [ ] CLI には可変長の入力リストを足さない。コマンドラインに 2 つのパスを並べれば、設定ファイルの場所・route の基準・画像を読んでよいディレクトリに答えなければならない（[roadmap.md](roadmap.md) 25.2）

**route の別名**（[roadmap.md](roadmap.md) 15.5）

- [ ] frontmatter の `aliases:` と AsciiDoc の `:sd-aliases:` により、古い hash route がページを描画し、hash が現在の route に置き換わる。アドレスバーには次回も生きているリンクが残る
- [ ] アンカーは置換をまたいで残る。アンカーはパスではなく見出しに属するからである
- [ ] 2 つのページが 1 つの別名を主張したらエラー。実在の route と衝突したら警告して実在の route が勝つ。判定の前に別名は正規化される——先頭のスラッシュ、拡張子の除去、`index` はディレクトリ
- [ ] 別名はサイドバーにも検索索引にも前後ナビゲーションにも届かない
- [ ] リポジトリの履歴から別名を生成しない。文書のリンク表が、どのクローンでビルドしたかに依存しないためである

**AsciiDoc の属性と読み込みの境界**（[roadmap.md](roadmap.md) 17.5）

- [ ] `sources.asciidoc.attributes` が `sectnums` のような体裁の属性と書き手独自の属性を、ロックではなく**既定値**として設定する。したがって自分で指定した文書が勝つ——Asciidoctor の API の挙動とは逆であり、設定ファイルが意味すべきことである
- [ ] `allow-uri-read` / `docinfo` / `backend` / `data-uri` / `imagesdir` / `source-highlighter` / `sd-*` は属性名と理由を告げて拒否する。`safe` と `base_dir` はそもそも受理しない。設定ファイルから広げられるサンドボックスは名ばかりだからである
- [ ] 実体パスが入力ルートの外へ解決される `include::` と画像を、解決先のパスを示して拒否する。Asciidoctor の safe mode がシンボリックリンクを解決しないため、テストは実際のシンボリックリンクを使う
