...
- `themes/default/layout.test.ts` … サイドバーの到達可能性（目次を末尾までスクロールしても検索欄が定位置に残ること、列が収まらない低い画面では目次を潰さずサイドバー全体のスクロールに戻ること）。到達可能性は要素の矩形ではなくヒットテストで判定する（矩形は祖先によるクリップを反映しない）。CSS のレイアウトそのものなので happy-dom では見えず、実 Chromium のある環境でのみ確認する
