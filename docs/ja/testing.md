...
- `build.pdfdensity.test.ts` … `pdf.density`（プリセット名の解決、`base` を土台に名指しした値だけを差し替えるオブジェクト形式、数値でも文字列でも受ける lineHeight、「数値と単位」でない値の拒否 — `calc(...)`・後続の宣言・未知のプリセット名・未知のキーを含む）。生成されたスタイルシートについては「書かれていないこと」を確認する。既定は規則を 1 つも出力せず、1 つの値だけを指定したオブジェクトは他の値を固定しない。この機能が存在する理由は枚数なので、実 Chromium があるときは同じ文書を各プリセットでビルドし、pdf-lib で枚数を読み戻して、段階が単調であることと、`normal` が無指定とまったく同じ紙に載ることを確認する
