...
- `build.pagebreaks.test.ts` … 改ページマーカー（v0.11）。core が足す規則は、セレクタを名指しするのではなく `@media print` ブロック全体として突き合わせる。「印刷の中にあること」も主張の一部にするためである。`#content` だけでなく `.page` も名指すのは、`style.css` を差し替えたテーマから奪えないのがそちらだからである。実 Chromium が あるときの主張は pdf-lib で読み戻した枚数で、同じ短い文書がマーカー無しで 1 枚、マーカー有りで 2 枚になることを Markdown（受理する 2 綴りとも）と AsciiDoc（`<<<`）で確認する。対照があることが 2 枚という主張を意味あるものに しており、同時に、短い文書の末尾に付いていた空白の紙の回帰テストでもある。もう 1 つの場合が `break-before` では なく `break-after` を選んだことを固定する。1 ページ目の末尾にマーカーがある 2 ページの文書は 2 枚であり、規則を 裏返せば 3 枚になる
- `sources/markdown/pageBreak.test.ts` … どの綴りが改ページマーカーかを固定する（v0.11）。受理する 2 形と その空白・引用符の揺れは monodocs が組み立てた要素になり、`<DIV>`・余分なクラス・2 つ目の属性・自己終了タグ・タグの間の空白・タグの間の文字・それ以上を含む `style`・コメントはすべて拒否して破棄したままにする。引用・リスト項目・表のセル・見出しの中のものはマーカーではない。同じファイルで、`<script>` と `onerror` 属性が今も 破棄されること（境界が動いていないという主張）と、マーカーが検索テキストに何も足さないことも確認する
