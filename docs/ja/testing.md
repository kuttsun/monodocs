...
- `report.test.ts` … `validate` が何を出すか（v0.11）。人間向けの報告は、行ごとに、その行がどのストリームへ出るかまで含めて確認する。機械可読な形を足す目的は散文を変えないことだからである（エラー、警告、サマリの順で、警告だけでも失敗する）。JSON の報告は標準出力に出る 1 つのオブジェクトで、`schemaVersion` と診断を持つ。所見が無いときも、あるときも、そこに出るのはそれだけである——ときどき JSON で、ときどき JSON のあとに文が付く出力は形式ではない。フラグが受け付けない形式は、受け付ける形式を挙げて拒否する。標準出力を解析しているジョブの前で散文へフォールバックしない
