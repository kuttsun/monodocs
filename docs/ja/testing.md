...
- `pipeline/pageBreakHeadings.test.ts` … `pdf.pageBreakLevel` がどの見出しに印を付けるか（v0.11）。規則は「どの見出しか」ではなく「その前に何があるか」なので、違いの出る場合を並べる。前にあるのがページタイトルだけ（印を付けない）、前に何も無い（付けない）、タイトルと見出しの間に導入文がある（付ける）、直前が手動マーカー（付けない。Chromium は強制改ページを畳まないため）。同じ集合を、平坦な Markdown の本文と Asciidoctor の `.sect1` / `.sectionbody` の入れ子の両方に対して走らせ、引用・表のセル・admonition・図の中の見出しが決して候補にならないこと（印刷用スタイルシートがそれらを分割しないため）も確認する
...
- `build.include-boundary.test.ts` … e2e（`include::` の読み取り境界を include processor の `handles` の中で検査する。ファイルへのシンボリックリンクとディレクトリへのシンボリックリンクがどちらも target と解決先を示して拒否されること、ネストした include も拒否されること、`validate` が `include/outside-input` を持つエラーとして報告すること、同じツリーを 1 階層上をルートにしてビルドすると外部の内容が実際に出力へ入る（穴そのものの主張）こと、内側に留まる include とシンボリックリンクは許されること、字句的な脱出は safe mode が jail 内へ復旧するので拒否しないこと、そして Asciidoctor が復旧したパスのほうを検査すること、存在しない include と循環 include で壊れないこと、コメントブロック内・偽の条件内の include は Asciidoctor が読まないのでそのままであること、属性参照から組み立てた target は展開済みで届くので検査できること、安全な include は Asciidoctor へ見送るので `lines=` と `tag=` が効くこと、include を引用したコードサンプルに触れないこと、バックスラッシュのエスケープを尊重すること、Markdown だけの文書に触れないこと）
...
- `diagnostics.test.ts` … 診断のモデル（v0.11）。コード集合については、利用者が頼る性質——重複が無いこと、並んでいること、綴りが 1 通りであること——と、どこからも報告されないコードを抱えていないことを確認する。後者はソースを検索して確かめるが、その前に登録簿自身の宣言を取り除く（そうしないと、どのコードも自分自身を見つけてしまう）。要になるのはエラー側の走査である。どちらのパッケージのソースでも `throw new Error(...)` があればそのファイルは落ちる。コードの無いエラーは、型の無い文として直列化するしかなく、それこそがこのモデルの取り除いたものだからである。派生クラスは許す（`MonodocsError` を継承し、自分のコードを固定しているため）。同じ境界へ届く他所からのエラーが、コードの無い所見ではなく `internal/unexpected` として出ることも確認する。そのうえで、ビルドが実際に何を報告するかを見る。壊れたリンク・存在しない画像・タイトルの無いページを持つ文書から出るすべての診断がコード集合のコード・severity・メッセージを持ち、解決できないリンクは文の中だけでなくフィールドとしても `index.md` と 5 行目を持つ——`formatSourceRef` が出口で潰していた位置である
- `pipeline/pageChecks.test.ts` … 文書を「組み立てる」のではなく「読む」ための 2 つの検査（v0.11）。見出しレベルの飛びは、見出しと前後のレベルを名指しして報告する。続きのレベルと、浅い方へ戻る場合は報告しない。ページの最初の見出しも、そのレベルが何であっても報告しない——タイトルを frontmatter に書いたページが `h2` から始まるのは正しく、そこを咎めれば、きちんと書かれた文書のほとんどを咎めることになる。`alt` 属性の無い画像は報告し、`alt=""` は報告しない。後半もケースとして置く。それが装飾画像を示す書き方だからである。最後のケースは検査を走らせる順序を固定する。埋め込みを有効にしても所見は base64 ではなく `pixel.gif` を名指しし、画像自体はきちんと埋め込まれる
- `report.test.ts` … `validate` が何を出すか（v0.11）。人間向けの報告は、行ごとに、その行がどのストリームへ出るかまで含めて確認する。機械可読な形を足す目的は散文を変えないことだからである（エラー、警告、サマリの順）。JSON の報告は標準出力に出る 1 つのオブジェクトで、`schemaVersion` と診断を持つ。所見が無いときも、あるときも、そこに出るのはそれだけである——ときどき JSON で、ときどき JSON のあとに文が付く出力は形式ではない。フラグが受け付けない形式は、受け付ける形式を挙げて拒否する。標準出力を解析しているジョブの前で散文へフォールバックしない。何が失敗になるかは独立したグループとして両形式で確認する。エラーは失敗、警告は失敗ではない、`--strict` を付けると警告も失敗、何も無ければどちらでも失敗しない——終了コードは報告が公開する severity に従う
- `documentMeta.test.ts` … 文書自身が名乗る情報（v0.11）。フッターの行は `version` / `date` / `authors` からその順に組み立て、バージョンの前の語は文書のラベル表から取る。したがって `lang: ja` なら「バージョン」になり、`html.labels` で `Rev.` にもできる——英語の既定だけを確認しても、語を直書きした実装に対して同じように通ってしまう。文書が言っていないものは、両側に何も無い区切りを出すのではなく落とす。空白だけの値も「言っていない」と数える。PDF の 3 つの欄は別に確認する。PDF の欄が 1 つしか無いので著者はまとめて `Author` へ、バージョンと日付は `Subject` へ、そして両方の値は**書いたまま** `Keywords` へ入る。PDF を並べたフォルダの検索に当たるのは `Version 1.2` ではなく `1.2` だからである
- `build.documentmeta.test.ts` … `document` の end-to-end（v0.11）。設定は解釈せずに読み戻し（日付は文字列であってカレンダーではない）、その行が生成 HTML のフッターへ届くこと、`lang: ja` では日本語になること、マークアップを含めばエスケープされることを確認する。フッターとは何かを固定するケースが 2 つある。`html.branding: false` では書き手の行が残り monodocs の行が消えること、何も設定しなければフッターはこの機能の前と同じであること。PDF 側は、ビルドがジェネレータへ渡すオプション（`author` / `subject` / `keywords`）を確認する。実際に配布されるバイト列は、実 Chromium がある環境で `build.pdf.test.ts` が pdf-lib で読み戻す。最後のケースはこの機能を縛っている条件である。同じ入力を 2 回ビルドすると同一のバイト列になる。フッターに載るのは書き手が書いた日付で、ビルドを実行した時刻ではない
