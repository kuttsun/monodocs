...
**検査は意図して過剰に行う。** 他人のパーサを模した静的検査は必ずずれ、緩い側へのずれはすべて穴に
なる。最初の実装は `////` のコメントブロックを追い、directive を行頭でしか照合しなかった。その隙間から
4 通りの文書が外部の内容を出力へ通した——listing ブロックの中の `////` が、Asciidoctor はそうでないのに
検査だけをコメント状態にし、`ifndef::x[include::y[]]` が検査の見ない場所に directive を置き、target 中の
`]` が Asciidoctor は受け付けるパターンで弾かれ、シンボリックリンクをたどった先で、Asciidoctor は字句
パスを保つのに検査だけが基準ディレクトリを変えた。そこでブロック構造は一切模さず、条件も評価しない。
テキスト中のすべての `include::` を、どこにあっても検査し、再帰は字句パスを保つ。その代償は誤った拒否
——コメントブロックの中、偽の `ifdef` の中、あるいはコードサンプルとして引用された include で、その
target がたまたまルート外の実在ファイルに解決される場合——であり、得られるのは、ずれが漏れではなく
ビルドの停止として現れることである。

**検査は静的に行い、`IncludeProcessor` は使わない。** 素直な実装は「target を検証して include を
Asciidoctor に返す拡張」だが、API がそれを許さない。`handles` は文書と target を受け取るものの
reader への経路が無く、`process` に渡されるカーソルは、文字列から変換した文書では include の解決
基準ではなく `.` を返す。したがって directive を引き取るとは、Asciidoctor 自身のパス解決に加えて
`lines` / `tag` / `tags` を再実装することであり、それは穴より大きな面であり、17.3 の「directive は
Asciidoctor に任せる」という約束にも反する。そこで target は変換の前にソーステキストから読み取り、
解決し、再帰的に（訪問済みを持って）検査する。

これが覆えないのは、Asciidoctor を走らせずには解決できない target——属性参照から組み立てたもの
（`include::{partialsdir}/x.adoc[]`）——である。推測せずに見送り、含意ではなくここに記録する。
字句的な脱出は safe mode が既に拒否していたが、出力に「Unresolved directive」を残す形だった。今は
検査のほうが先に捕まえるので、ルートを出る 2 つの道はどちらもビルドを止め、同じことを言う。
