...
外の内容を出力へ持ち込んだ。一方 `../` と絶対パスは拒否された。したがって、アーキテクチャ文書の
...
**この検査が前提にしていること。** Asciidoctor は `handles` が最初に true を返した include processor
だけを使い、`prefer()` でグローバルに登録された processor はこの境界より前に並ぶ——そちらが include を
引き取れば、境界は尋ねられない。monodocs は他の processor を登録しないので自身の CLI に影響は無い。
問題になるのは、`@monodocs/core` を組み込んだプログラムが Asciidoctor のグローバル拡張を登録している
場合である。この境界自身も `prefer()` を呼び、普通に登録されたものよりは前に出る。
