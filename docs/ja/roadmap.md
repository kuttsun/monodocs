...
pdf:
  ...
  # 印刷時の版面の密度（v0.10）: normal（既定）/ compact / tight。または base と
...
しない。そして異体字シーケンス・結合文字・絵文字 ZWJ シーケンスは、構成する各コードポイントが単独
では描けても合成に失敗し得るので、コードポイントを個別に測る方式は、この検査を必要とさせている
まさにその場合について成功を報告してしまう。計算後フォントと組にするのは、本文・コードブロック・
カスタムテーマが同じ family に解決するとは限らないからである。検査は `document.fonts.ready` の後に
走らせ、テーマが data URI で持つ webfont は「ある」として数える。
...
保証ではない。そこで検査は、別の面の私用領域コードポイントを使って自分の基準をまず検証する。2 つが
食い違えば、このマシンには私用領域を描く何かが入っており比較が成り立たないということなので、
根拠を持てない指摘を出す代わりに、この環境では検査を実行できないと報告する。
...
同梱するプリセットは 3 つ。枚数を決める値をまとめて動かす。1 つだけ動かしても組版として整わない
...
見出し・段落・箇条書きと 24 行の表からなる日本語の事業書類で、それぞれ 5 枚・3 枚・2 枚になる。

**`normal` は何も出力しない。** これは書き出すべき値の集合ではなく、スタイルシートが既にして
いることの記録だからである。密度を指定しない文書は以前と 1 バイトも変わらずに印刷される。読者が
ブラウザから HTML を印刷するとき、その人自身の基準文字サイズを継承することも含めて。「normal」の
名のもとに 16px を打ち込めば、それを取り上げることになり、得るものは何も無い。オブジェクト形式の
中でも同じ規則が働く。`normal` と異なる値だけが書き出される。
...
当初からここに仕様として書かれていて、実装は v0.10 で行う。それまで CLI が持っていたのは `build` /
`watch` / `serve` / `validate` の 4 つだけで、この章は存在しないコマンドを説明していた。

書き出すのは上の 2 ファイルだけで、既にあるものは**上書きしない**。見つけたものを名指しし、何も
書かずに終える。既にファイルの入っているディレクトリで実行しても作業を壊せないようにするため
である。生成する
設定は全キーのダンプではなく、コメント付きの短い出発点にする。ダンプはオプションが増えるたびに
生成し直す必要があり、しかも理解していないキーを抱え込むことを読者に教えてしまう。残りは
ドキュメントサイトの設定ページを指す。コメントはメッセージ言語（25.6）に従う。
...
- `pdf.density` の `normal` / `compact` / `tight` が、同じ文書を段階的に少ない枚数へ収める。
  オブジェクト形式はプリセットを土台にするので、1 つ値を変えるために残りを書き写す必要が無い。
  既定は規則を 1 つも出力せず、密度を指定しない文書はこれまでと同じに印刷される
