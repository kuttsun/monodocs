...
`remark-rehype` に渡す前に 2 つの厳密な綴りと突き合わせ、一致したものを monodocs が組み立てた要素
...
`<DIV>`、`class="page-break foo"`、2 つ目の属性、`<div class="page-break"/>`、それ以上を含む `style`
は、修復せずに拒否する。それらは Markdown の raw HTML がこれまでそうだったもの——破棄——のままで
ある。
...
`break-after` ではなく `break-before: page` を使う。ページ末尾のマーカーが空白の紙を残さないため
であり、ファイル境界が after ではなく before で改ページしているのも同じ理由である。
