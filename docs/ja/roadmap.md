...
# 文書自身が名乗る情報（v0.11）。すべて任意で、いずれも解釈しない。日付は解析せず、
# バージョンは何とも比較しない（13.5）。既定では未設定。
# document:
#   version: "1.2"
#   date: "2026-08-22"
#   authors:
...
# 相対的なものすべての基準になるディレクトリ（v0.12）。既定は input の値なので、書かなければ意味は
# 変わらない。input と併記できるのは同じディレクトリを指すときだけで、複数のディレクトリにまたがる
# 文書は sources.include で選ぶ（12.5）。
# root: "."
...
sources:
  ...
  asciidoc:
    extensions:
      ...
      - ".asciidoc"
      - ".asc"
    # ロックではなく既定値として設定する Asciidoctor 属性。文書が自分で指定すればそちらが勝つ
    # （v0.12）。読み込み先を動かす属性とサンドボックスそのものは、属性名と理由を挙げて拒否する（17.5）。
    # attributes:
    #   sectnums: true
    #   product: "Widget"
  # ページ化しうるものを選ぶ、root からの相対 glob（v0.12）。書かなければ root 配下すべてが候補。
  # exclude はここから引き、しかも最後に引く（12.5）。
  # include:
  #   - "README.md"
  #   - "docs/**"
...
- バージョン間での**バイト単位の同一出力**は約束しない。生成物には生成器のバージョンが載り、
  ...
  同じ monodocs のバージョンなら同じ HTML のバイト列になる**ことであり、これがコミットした成果物を
  差分でレビューできるということである。ビルドが自前の時刻を埋め込まない理由でもある（13.5）。
  PDF はこの約束の外である。実測: Chromium が自分の作成日時と更新日時をファイルに書き込み、
  monodocs は日付を足しもせず、Chromium のそれを削りもしない
...
文書が設定するものであり、両者は同じキーを違う距離から見たものである。したがって両方書けるのは
同じディレクトリを指すときだけで、それ以外はマージではなく設定エラーとする。これは「`root` の外なら
エラー」より厳しく、意図してそうしている。`root` の**内側**の下位ディレクトリを指す `input` に、
include を長く書き下したもの以外の意味は無く、あるとすれば姿を変えた 2 つ目のルートである。この規則は
コマンドラインにも及ぶので、`root: "."` の設定に対する `monodocs build ./docs` は、どちらかを黙って
選ぶのではなく止まる。ファイルを指す `input` は、それを含むディレクトリとして比較する。設定ファイルを
探すときに既にそう扱っている（25.2）。

**`input` を書かずに `root` を書いた場合、ビルドが指すのは `root` である。** 既定の `./docs` へ
落とすと、そのようなディレクトリを持たないリポジトリで `root: "."` が失敗する。このキーが存在する
理由になっている、まさにその形のリポジトリで。

**否定パターンはどちらのリストでも拒否する。** picomatch は配列のパターンを OR で結合するので、
`["docs/**", "!docs/drafts/**"]` はすべてに当たる——2 つ目が `docs/drafts` の外のすべてのパス、
つまりほとんどに当たる——ため、下書きは束に残ってしまう。枝刈りはさらに悪い。パターンの静的な
接頭辞を読むので、`!foo/**` ではそれが `foo` になり、書き手が外したかったディレクトリにちょうど
入り、残したかったほうを飛ばす。2 つのキーが既に両方を述べているのだから、その一方を言い直す
3 つ目の方法は、誤読させるのではなく拒否する。

**`root` はディレクトリでなければならない。** route も画像も `include::` もそこから解決される
のであり、ファイルはそれになれない。検査しなければ半分だけ動く——1 ページはビルドでき、相対パスの
画像を足して初めて基準がファイルだと分かる——が、それは 12.2 が防ごうとしている
「受理されて無視される」失敗である。存在しない `root` はビルドに任せて「見つからない」と報告させる。
存在しない `input` も同じで、無いファイルと無いディレクトリは区別しようがなく、比較すれば
「ファイルが無い」を「ルートが食い違う」に化けさせてしまう。

**組み込みの除外パターンはルートを基準に固定されている。** つまりルートを動かすと、それらが覆う
範囲も動く。`_partials/**` はルート直下のディレクトリに当たり、`**/_*` は名前が `_` で始まるファイルに
当たる。したがって `root: "."` のもとでは `docs/_partials/` は前者に当たらない——現在ネストした
`sub/_partials/` が当たらないのと同じことである。これはこのキーが開ける穴ではなくルートが動いたことの
帰結であり、文書が本当に望むものを述べる場所は `sources.exclude` である。

### 12.6 段落内の改行（v0.13）

段落の中の改行には、成り立つ解釈が 3 つある。そして monodocs はそのうち 1 つしか出したことがない。
CommonMark は空白にする——これが現在の monodocs である。`<br>` にするレンダラもある——GitHub は
issue のコメントでそうするし、Typora、GitLab、VS Code のプレビューはいずれも設定として持っている。
そして CSS Text は、東アジアの文字に挟まれた改行は消えると言っている——一文一行で書かれた日本語の
段落が、1 つの段落として読めるように。この 3 つは同じものの程度差ではない。それぞれ別の問いに
答えており、書き手がどれを望むかは文書からは決められない。

```yaml
sources:
  lineBreak: "space" # space（既定） / break / join
```

| 値                | 段落内の改行は                     | Markdown                          | AsciiDoc                              |
| ----------------- | ---------------------------------- | --------------------------------- | ------------------------------------- |
| `space`（既定）   | 空白になる                         | 何もしない                        | 何もしない                            |
| `break`           | `<br>` になる                      | 改行を `break` ノードにする       | `hardbreaks-option` を既定値として渡す |
| `join`            | 東アジアの文字に挟まれていれば消える | 共有ヘルパ 1 つ                   | 同じヘルパ                            |

**キーは `sources.markdown` ではなく `sources` の直下に置く。** `join` は構文についての規則ではなく
文字についての規則である。AsciiDoc は CommonMark とまったく同じように段落の行を連結するので、
Markdown にしか届かないキーは、混在文書（6.3）の半分だけを、書き手が選んでいない理由で別の読み方に
してしまう。`sources.exclude` と `sources.excludeDefaults`（12.3）が同じ理由で既にその高さにある。

**既定は `space` のまま**とする。既存のすべての文書が現にそれで組まれている。今ある出力を 1 バイトも
変えない。ある桁で折り返された段落を壊さない——`break` はそれを短い行の山に変える。しかも壊れ方は
非対称である。改行が足りないことは、それを望んだ書き手には見えている。要らない改行は、問題の
なかった文書の全体に現れる。そして GitHub がリポジトリ内の `.md` を表示する仕方と一致する。同じ
ファイルがあちらで読まれ、こちらで組まれる以上、これは効いてくる。

**根拠に数えないもの**が 1 つある。準拠である。CommonMark も GFM も、soft break を空白にすることも
改行にすることも許している。したがって `break` は 6.1 が約束する GFM 対応からの逸脱ではない。既定を
決めているのは GitHub の表示であって、仕様ではない。

**`join` が適用する規則は、現行の仕様が義務づけているものではない。そしてエンジンの実装は割れて
いる。** CSS Text Level 3 §4.1.3 も CSS Text Level 4 も、畳める segment break は「文脈に応じて
空白（U+0020）に変換されるか削除されるかのいずれかである」とし、「その操作の規則はこのレベルでは
UA 定義である」と述べている。両側の East Asian Width が F / W / H でどちらもハングルでなければ
削除する、という規則が規範だったのは 2013 年の Working Draft であり、そして
web-platform-tests が今も主張しているのはその規則である。
`segment-break-transformation-rules` の 49 件のうち 9 件が Chrome 152 と Safari 26.6 で失敗し、
Firefox 154 では 1 件も失敗しない（いずれも stable、2026-08-31 時点）。開発イメージでも実測した
（Chrome/151.0.7922.137、本文 16px）。`日` + 改行 + `本` は 35.59px、`日本` は 32.02px——3.58px の
空白である。同じ 3.58px が `。` と `次` の間、全角ラテン文字どうしの間、半角カナどうしの間にも
現れる。

つまり `join` とは、仕様が UA に委ねた 2 つの挙動のうち一方を monodocs が選び、ビルド時に一度だけ
選ぶことで、答えがどのエンジンで開いたかに依存しなくなる、ということである。それが Firefox の
実装であり、テストが主張しているものであることは、選ぶ価値がある理由である。何もそれを義務づけて
いないことは、これが既定ではなくキーの値である理由である。

これはこのリポジトリにとって仮定の話ではない。`examples/ja/search.md` は一文一行で書かれており、
Markdown パイプラインはその改行をそのまま HTML に出す。したがって公開している日本語サンプルは——
そしてそこから作られる、定義上 Chromium の PDF は——文と文のあいだすべてに空白を抱えている。

**`break` は AsciiDoc には属性で届き、既定値として設定される。** 属性は `hardbreaks-option` で、
`hardbreaks` は受理される別名である。17.5 は、monodocs が設定する属性はロックではなく文書側が
上書きできる既定値であることを要求している。その仕組みは仮定ではなく実測した。`@asciidoctor/core`
4.0.6 で、そして 4.0.11 でも変わらないことを測り直したうえで、API から `""` として渡した属性は
文書側の `:hardbreaks-option!:` を跳ね返し、同じ属性を
`"@"` として渡すと跳ね返さない。`@` サフィックスこそが 17.5 の規則の中身であり、このキーがその
最初の利用者になる。

そこから生じる非対称は、隠さずに書いておく。AsciiDoc の文書は自分だけ `break` を切れるが、Markdown
の文書はできない。AsciiDoc に文書属性があるのは AsciiDoc に文書属性があるからであって（17.5）、
Markdown に対応物を発明することは、17.5 が既に断ったテンプレート言語そのものである。

**`join` はテーブルを必要とする。** JavaScript の正規表現は `\p{Script=Han}` は持つが
`\p{East_Asian_Width=W}` は持たない。したがって F / W / H の範囲は Unicode のデータファイルから
ソース中のテーブルに生成し、Unicode のバージョンを記録し、テーブルがそれと一致し続けることを
テストで確かめる。`pre` と `code` は対象外とする。そこでは `white-space: pre` により、改行は
segment break ではなく書き手が引いた行であり、消せばコード例の 2 行が繋がってしまう。

**どちらの値も、ページのテキストを収集する前に、各 renderer の内側で適用する。**
`postprocessPages` は `page.html` を読み直すが、renderer が先に作った `page.text` は作り直さない。
共有の後処理でやると、検索インデックスだけが HTML とは違う文書を語ることになる。2 つの renderer から
1 つのヘルパを呼ぶ形なら両者は一致し、Source Renderer の境界（11 章）も壊れない。

**各値が検索に何をするか。** `break` は text ノードを分割する。結果リストは分割をまたいで一致する
一方、22.5 の本文ハイライトは 1 つの text ノードの中で動くのでそこを marking できない——明示的な
`<br>` に既にある制約だが、`break` はその頻度を大きく上げる。`join` は逆方向に動く。そして取り除く
ものは HTML が見せているものとは違う。HTML は改行を抱えているが、ページのテキストを収集する時点で
`hast-util-to-text` がそれを空白に畳むので、インデックスが持っているのは `です。 次` であり、2 つの
文にまたがるクエリは何にも当たらない。`join` はその収集より前に改行を取り除くので、同じクエリが
当たるようになる。

**既定は `lang` に従わせない。** 日本語だと宣言した文書は、Markdown の意味が変わる文書ではない。
従わせれば、同じファイルが表示上の設定次第で違う構造を作ることになるし、1 つの文書が両方の言語を
含むこともある。`lang` が選ぶのはラベルと `<html lang>` であって（23.4）、パーサではない。

**これは 1.0 を待たせない。** 既定が何も変えない任意のキーであり、12.4 が minor での追加を許して
いるものである。v0.12 の後に置くのは、`break` の AsciiDoc 側が 17.5 の定める属性の仕組みであり、
それが先に存在している必要があるからである。
---
...
`version` は何とも比較しない。文字列に対して唯一行うのは前後の空白の除去で、空白だけの値は
フッタの空行ではなく未設定として扱う。これらがすることは 3 か所へ届くことである。PDF の文書情報
...
**どのフィールドがどこへ届くか。** フッターは 1 行を持つ——`バージョン 1.2 · 2026-08-22 ·
ドキュメントチーム`。バージョンの前に付く語は `lang` が選ぶラベル表から取るので文書の言語に従い、
`html.labels` で差し替えられる。PDF の文書情報は、著者を `Author`、バージョンと日付を `Subject`、
書き手が書いたままの両方の値を `Keywords` に取る。PDF を並べたフォルダの検索に当たるのは
`Version 1.2` ではなく `1.2` だからである。フッターの要素は `html.branding: false` でも残る。
消えるのは monodocs の行であって書き手の行ではない——たまたま同じフッターを共有していた、別々の
主張である。

**再現性を実測した結果。** 生成される HTML は、同じ入力からのビルドでバイト単位に同一であり、
テストがそれを主張する。PDF はそうならない。そして原因は monodocs ではない。Chromium が自分の
`CreationDate` と `ModDate` を、圧縮ストリームの中に書き込む。monodocs は日付を足しもせず、
それを削りもしない。したがって 12.4 の約束は、それが書かれた対象——差分でレビューできる、
コミットした `docs.html`——について成り立つ。PDF が時刻で違うことは、後から発見されるのではなく
ここに記録しておく。
---
...
とは「文書が別のことを言わない限り全文書がこうなる」を述べる場所である。その仕組みは Asciidoctor
自身のものであり、仮定ではなく実測してある。値の末尾の `@` が soft set を意味し、
`@asciidoctor/core` 4.0.6（4.0.11 でも変わらないことを測り直した）では `""` として渡した属性は
文書側の `:name!:` を跳ね返し、`"@"` として
渡すと跳ね返さない（12.6）。

**名前は分類の前に検証する。** 書かれたままのキーと突き合わせる denylist は denylist ではない。
Asciidoctor はキー自体の装飾を読む——`name@` は soft set、`!name` / `name!` / `name!@` / `!name@` は
unset——そしてそのどれもが、裸の名前で当の属性に届く。実測した。`allow-uri-read@` は
`allow-uri-read` を含むリストを素通りし、ビルドが HTTP で URL を取りに行って応答を出力に入れた。
この分類がまさに防ぐために存在するものである。したがって名前は装飾のないもの——英小文字・数字・
アンダースコア・ハイフン——でなければならず、装飾された名前は剥がすのではなく拒否する。剥がすとは
その装飾が何を意味したかを決めることであり、答えは常に「このキーが提供しないもの」だからである。

**`outfilesuffix` と `relfilesuffix` も「そもそも設定させないもの」に入る。** これらは Asciidoctor が
相互参照の末尾に付けるものを決め、リンク変換は既知の拡張子の集合と照合して `xref:b.adoc[]` を
hash route に変える。実測した。`outfilesuffix: ".xyz"` は単一 HTML の中に `href="b.xyz"` という
文字列をそのまま残す——自己完結しているはずの文書の中の死んだリンクであり、しかも `validate` は
何も報告しない。壊れたリンクらしく見えるところが何も無いからである。

**実装して分かった、分類への 3 つの追加。** `showtitle` は「そもそも設定させないもの」に入る。
monodocs がこれを設定しており、切ると、ページタイトル・見出し一覧・すべての要素 ID の元になっている
`h1` が消えるからである。`docdir` / `docfile` / `docname` / `docfilesuffix` / `outdir` も同じ組に入る。
`base_dir` がそこにあるのと同じ理由で、これらは Asciidoctor が読んでいるファイルから設定するもので
あり、外から設定すればパスの解決先が動く。そしてこのキーは **unset を提供しない**。値 `false` と
末尾が `!` の名前は拒否する。設定ファイルは「文書が別のことを言わない限り全文書がこうなる」を述べる
場所であり、個別に外すのは文書側が `:name!:` と書くことだからである。末尾が `@` の値も拒否する。
それは monodocs がここのすべての値に付ける印であり、自分で書けば二重に述べることになる。
...
実測した。しかも主張が示唆するより悪い——シンボリックリンクされたファイルもディレクトリも、jail の
外の内容を出力へ持ち込んだ。一方 `../` と絶対パスは、拒否されるのではなく jail の中へ**復旧**された
——復旧先に何も無いときに読者が見るのは Asciidoctor 自身の「Unresolved directive」であり、これは
拒否と見間違えやすく、実際に見間違えた。したがって、アーキテクチャ文書の
「外部アクセスを防ぐ」という記述は強すぎる。v0.12 は表現を弱めるのではなく、記述が真になるようにする
——include したファイルの実体パスを入力ルートと突き合わせ、外へ解決されるものは、解決先のパスを
示して拒否する。同じ検査を画像（20.2）にも掛ける。まったく同じ穴がそこにもあるからである。

**検査はテキストを読むのではなく Asciidoctor に尋ねる。** include processor の `handles` は、
読もうとしているすべての include について、文書と**展開済みの** target を伴って呼ばれる。`false` を
返せばその include を見送り、Asciidoctor が処理する——したがって `lines` / `tag` / `tags` をはじめ
17.3 が約束するものはそのまま効く。止まるのは、実体パスがルートの外へ落ちる target だけである。

最初の実装はソーステキストを走査した。processor には文書から reader への経路が無いと考えたからで
ある。それは誤りで、しかも小さな理由による誤りだった。`doc.getReader()` は `PreprocessorReader` を
返し、`reader.getCursor().getDirectory()` が include の解決基準となるディレクトリで、ネストの各段で
正しい。ただし `getDirectory` は reader ではなくカーソルにあり、メソッドが無いことを経路が無いことと
読み違えた。その結果の走査は、`////` の行が何を意味するか、directive が行頭以外に置けるか、target に
`]` を含められるかを自分で決めることになり、その 3 つすべてを緩い側へ間違えた——どれを通っても外部の
内容が出力に届いた。過剰検査はそれらを塞いだが、コメントブロックや偽の `ifdef` の中の include を
誤って拒否する代償を伴い、なお属性参照から組み立てた target は解決できなかった。Asciidoctor に
尋ねればそのどれも起きない。include が起きるまさにその瞬間に尋ねているからである。

**パスの解決も Asciidoctor に任せる。** target を尋ねるだけでは足りない。safe mode は jail の外へ
登る target を拒否せず、`..` を捨てて**復旧**し、復旧後のパスを読む。実測では、jail が `root/docs` の
ときの `include::../x.adoc[]` は `root/x.adoc` ではなく `root/docs/x.adoc` に解決される。したがって
素直に解決した検査は存在しないパスを見て見送り、ツリーの外を指すシンボリックリンクが読まれた。
`normalizeSystemPath` は Asciidoctor 自身が呼ぶものなので、もう食い違う余地が無い。同じ修正で、
逆向きの誤った拒否——復旧後にルート内へ収まるパスを拒んでいた——も消える。

したがって字句的な脱出は脱出ではない。safe mode がそれを jail の中へ復旧し、そこに何も無ければ
読者が見るのは Asciidoctor 自身の「Unresolved directive」である。

**この検査が前提にしていること（正確に）。** 見ているのは、Asciidoctor 自身の include 処理が読む
であろうパスである。他の include processor はそれを 2 通りに変えられ、どちらもこの境界からは見えない。
`handles` が**先に** true を返して include を引き取る場合と、**後で** true を返して別のファイルの中身を
push する場合である。したがって前提は「preferred な processor が無いこと」ではなく、より広く
「他の include processor が、この境界を追い越すことも、尋ねられた target とは別の場所を読むことも
しないこと」である。

monodocs は他の processor を登録しないので、自身の CLI・`watch`・`serve` に影響は無い。問題になるのは、
`@monodocs/core` を組み込んだプログラムが Asciidoctor のグローバル拡張を登録している場合である。
この境界は `prefer()` を呼び、普通に登録されたものより前に出る——実測では、registry へ直接登録され
グローバル拡張はその後で activation されるため、`prefer()` が無くても既に前にいた。`prefer()` が実際に
買っているのは、先に activation された別の `prefer()` する processor より前に出ることであり、最後に
activation されたものが依然として先頭を取る。
...
走らせるので、`validate` が報告する検査はすべてビルドも報告し、同じ検査を 2 か所に持たない
（architecture.md）。逆は成り立たない。そう書いていたのは雑だった。PDF を書き出すビルドは、その
作業でしか分からないことも報告する——ページ番号の帯に対して足りない余白、このマシンのどのフォントも
描けない文字、pre-render に失敗した図である。Mermaid の `pre-render` は `client` に強制してブラウザを起動しない。つまり
図の構文エラーは対象外であり、走らせていない検査をほのめかす代わりに、そう明言する。

**何が失敗になるか。** エラーは失敗にする。警告は失敗にしない——`--strict` を付けたときだけ
失敗する。これは v0.11 が出る前にここへ書いていた判断（「何か見つかれば非ゼロで終了するので、
足すべき `--strict` は無い」）を反転させたものであり、理由は 12.4 である。マイナーリリースは検査を
増やしてよく、追加は「それを使っていない設定を壊しようがない」はずだった。しかし新しい警告は、
誰も触っていない文書に対して、緑だったジョブを赤にしていた。終了コードで 2 つの severity を
平らにすることは、報告の側と矛盾してもいた。報告は利用側が判断するために `severity` を公開して
いるのに、`warning` と読んだジョブがプロセスの死を見ることになる。警告をゲートにすること自体は
価値があり、それは唯一の挙動ではなく決定——`--strict`——になった。既定の変更はメジャーリリースで
しか行わない（12.4）ので、1.0 より前に取った。

`validate` に足りなかったのは機械が読める形である。プルリクエストに注釈を付けたい CI は、翻訳
された散文を解析するしかなく、それは言語によって変わり、文言を直せば変わった（12.4）。
...
**`alt` の検査が実際に見えるもの。** 実装後に両形式で実測した。Markdown の `![](x.png)` は
`alt=""` になる——これは装飾を示す書き方であり、意図して指摘しない。Asciidoctor は alt をファイル名
（拡張子を除いた部分）から導出するので、`image::x.png[]` は `alt="x"` になる。つまり属性そのものが
無い画像が出力へ届くのは、AsciiDoc の passthrough ブロックからだけである。検査が当たるのはそこで
あり、そこはどちらの変換器も守っていない唯一の経路である。テーマ自身のマークアップは対象外である。
対象だと書けば、それはこのマイルストーンが取り除くために存在するドリフトそのものだった。検査は
各ページの本文に対して走り、テンプレートはそのあとで適用される。`alt="x"` を
「ファイル名から導出された」として報告する手は無い。書き手が書いたのかどうかは出力からは分からない
からである。したがってこの検査は、それを規定した文よりも狭い。覆う範囲はここに書いたとおりである。
...
このマイルストーンは、予算とは無関係なもう半分を抱えている。そのことを取り繕わずに書いておく。
v0.11 が改ページと 1.0 の契約でそうしたのと同じである。`sources.lineBreak`（12.6）がここにあるのは、
`break` の AsciiDoc 側が v0.12 の作る属性の仕組みを必要とし、かつこのキーに依存する後続が無いから
であり、そしてこのリポジトリが公開している文書一式が現に影響を受けているからである——`examples/ja`
は一文一行で書かれており、文と文のあいだすべてに空白が入った状態で出ている。
...
- `sources.lineBreak`（`space`（既定）/ `break` / `join`）を追加する。ページのテキストを収集する
  前に、両方の renderer の内側で適用する（12.6）
...
- 画像を再エンコードしないという決定が、理由——バイナリが載せられないネイティブ依存、HTML ビルドが
  ...
  のと同じように
- `sources.lineBreak: break` が両形式で段落内の改行を `<br>` にし、`:hardbreaks-option!:` と書いた
  AsciiDoc 文書はそれでも勝つ。属性が 17.5 の要求する `@` サフィックスで soft set されるからである
- `sources.lineBreak: join` が、East Asian Width が F / W / H でどちらもハングルでない文字に挟まれた
  改行だけを取り除き、それ以外はそのままにし、`pre` と `code` の中には一切触れない。範囲は Unicode の
  データファイルから生成し、Unicode のバージョンを記録し、テーブルがそれと一致し続けることをテストが
  主張する
- 既定の `space` が直前のリリースと同じバイトを出し、既存の fixture をそのままビルドするテストが
  それを示す
- 3 つの値のいずれでも `page.text` と HTML が一致し、検索結果がページに無いテキストを指すことがない
