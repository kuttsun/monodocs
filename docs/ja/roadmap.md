...
v0.15 はこれに意見ではなく実測で答える。実際の数式を並べた見本文書を HTML と PDF に組み、対応する
...
**この例はテストの fixture である（v0.12）。** それまでは散文だったので、実際にずれた。
...
したがって、アーキテクチャ文書の「外部アクセスを防ぐ」という記述は強すぎる。v0.13 は表現を弱める
...
1 回の描画で 1 枚だけフッタを抑制できるかどうかは v0.15 が実測する。できなければ、表紙をヘッダも
...
**ディレクトリが複数あるとき（v0.13）。** 入力引数が指すのは今後もパス 1 つで、複数にまたがる
...
**`validate` とは何か（v0.12）。** 何も書き出さないビルドである。`build` と同じ `preparePages` を
...
何か見つかれば警告だけでも既に非ゼロで終了するので、足すべき `--strict` は無い。足りないのは機械が
読める形である。今はプルリクエストに注釈を付けたい CI が、翻訳された散文を解析するしかなく、それは
言語によって変わり、文言を直せば変わる（12.4）。

```bash
monodocs validate --format json
```

JSON は診断モデル（27.3）を直列化したもので、自身のスキーマバージョンを持つ。同時に検査を 3 つ
足す。いずれもパイプラインが既に持っている情報から判定できるものを選んだ。

- **見出しレベルの飛び**——`h2` の次が `h4`。ページ内目次（22）を壊し、見出しレベルで移動する
  支援技術のすべてを壊す
- **`alt` 属性が無い画像**。明示的な空の `alt=""` は指摘しない。書き手が装飾画像だと示す書き方で
  あり、ライトボックスも既にその区別を尊重している（23.2）
- **別ファイルへのリンクのアンカーが存在しない**——今もビルド時に警告しており、つまり既に分かって
  いる。散文の 1 行ではなく、コード付きの診断として報告に載る
...
エラーと警告は文字列である。`validateSite` は `errors: string[]` と `warnings: string[]` を返し、
CLI は接頭辞を付けて出す。モデルはそれだけである。唯一の利用者が端末を読む人間だったから、それで
足りていた。
...
パイプラインは口に出しているより多くを知っている。いくつかの警告では `formatSourceRef` がファイルと
位置を散文に組み立てており、つまり位置は存在していて、出口で潰されている。これは見つける仕事では
なく、捨てない仕事である。
...
この文書をもう一度真にし、番号を名乗る前に 1.0 が実際に何を約束するのかを書く。v0.11 は仕様を先に
書いて実装がそれに一致した機能を出した。その同じファイルの別の場所で、設定例はスキーマに無いキーを
説明し続け、architecture.md はコードが持たなくなったリンクの挙動を説明し続けていた。誰も確かめない
場所で誤っている仕様は、短い仕様より悪い。以後のあらゆる決定がそれを引用する。

もう半分は、このマイルストーン以降のすべてが必要とするモデルである。診断は今は文字列であり、
だから `validate` は CI に渡せるものを持たず、だから「JSON 出力を足す」は小さな変更ではなく、
先に存在していなければならないデータモデルである。

実装範囲：

- 仕様をコードへ同期させる。設定例（12.1）、architecture.md の別ファイルアンカーの段落、そして
  status.md の一覧表——v0.11 のチェックが全部埋まったあとも planned と言い続けている
...
- 両言語のドキュメントが、ツールを在るがままに記述している。リファレンスにスキーマの持たないキーは
  無く、architecture.md にコードのしない挙動は無い。v0.12 がそれを真にし、1.0 はそれをもう一度
