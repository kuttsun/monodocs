...
- monodocs が印字するもの（Commander が生成する見出しを含む `--help`、すべてのエラー、すべての警告）は、ひとつのカタログを通します。既定は英語で、`--lang ja` または `MONODOCS_LANG=ja` で日本語になり、フラグが環境変数に優先します。`LANG` / `LC_ALL` は意図的に見ません。ビルドログを、それを作ったマシンに依存させないためです。core はコードを返すのではなく現在の言語を保持するので、公開している `warnings: string[]` と `Error.message` の形は変わりません。依存パッケージのメッセージが包まれずに届くものは対象外です。ただし読者が実際に当たる引数エラー（不明なオプション・不明なコマンド・引数の欠落）は捕まえて訳します。放っておくとパーサが自分で終了するので、後段では手が出せません。カタログの外で新しい文字列を出すとテストが落ちるようにします（穴を後から見つける形にしない）。文書の `lang` とは別物で、あちらはページを、こちらは端末を記述します。
