...
- 0.x 期間は `latest` が指すバージョンだけをサポートする。旧版で見つかった脆弱性には、
  バックポートではなく修正を含むバージョンへのアップグレードで応える（[SECURITY.md](../../SECURITY.ja.md)）。
...
- [x] サポート範囲を明記し、SLA は提供しないことを明記する。
...
- patch: 後方互換のバグ修正。
- minor: 機能追加。0.x では破壊的変更を含むこともあり、その場合は Release notes に記載する
  （0.10 は `<html lang>`・印刷時の版面・未知の設定キーの扱いを変えた）。
- major: 破壊的変更。1.0 とそれ以降の互換境界のために取っておく。
- `next`: プレリリース。
- `latest`: 安定版。

  0.x 期間のサポート対象は `latest` が指す単一の安定版だけである。修正は前方にのみ出し、旧版には
  バックポートではなく修正を含むバージョンへのアップグレードで応える。その理由と、旧版の報告者に
  何を約束するのかは [SECURITY.ja.md](../../SECURITY.ja.md) に書いてある。

  前方のみとしたのは、「重大なものは過去 minor にも patch を出す」という以前の約束が、この体制では
  守れないものだったからである。バックポートは古いタグへのコミット 1 つでは終わらない。ブランチ、
  タグ、Release、`latest` を動かさない npm publish、単体バイナリ 2 本、`verify-published.yml` と
  `verify-release-binaries.yml`、そして Node.js の無い Linux と Windows の実機での手動パス——
  つまりリリース 1 回分まるごとを、単独メンテナが緊急時に走らせることになる。同じリリースごとの
  コストが Docker と VS Code 拡張を見送らせた（[roadmap.md](roadmap.md) 8.3、v0.7）。セキュリティの
  約束にはより厳しい基準を当てるべきであって、その逆ではない。守れない約束は、「アップグレードが
  対処である」と明記することより悪い。それが許容できるのは、monodocs がビルド時 CLI であり、
  出力が静的ファイルだからである。誰かの製品に組み込まれるライブラリではないので、アップグレードは
  移行作業ではなくビルドの再実行で済む。
...
- [x] Node.js と Chromium の対応範囲を見直す。（四半期チェックリスト。直近の期限は 2027 年 4 月の
      Node 22 LTS 終了）
- [x] dist-tag がサポート方針どおりの位置を指しているか確認する。（四半期チェックリスト。サポート
      対象が 1 つなら別途 EOL を告知するものは無い。安定版の公開そのものがサポートの移動である）
