...
| Published package      | `.github/workflows/verify-published.yml`, run by hand against a dist-tag or version. Steps that verify a feature added in 0.10 or 0.11 are gated on the installed version, so an older release can still be verified |
