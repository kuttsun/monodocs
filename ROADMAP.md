...
`single-docs` は、複数の Markdown / AsciiDoc ファイルから、単一の HTML または PDF ドキュメントを生成するツールである。
...
`single-docs` は Pandoc の代替を直接目指すものではない。
...
`single-docs` では、入力ファイルの管理は分割されたままにし、出力だけを単一ファイル化する。
...
ただし、初期段階では `single-docs` のみでよい。

---

## 5. 基本コンセプト

`single-docs` は、複数のソースファイルを一度共通の `Page` モデルに正規化し、その後に HTML / PDF へ出力する。
...
```bash
single-docs build ./docs --format html -o ./dist/manual.html
```
...
```bash
single-docs build ./docs --format pdf -o ./dist/manual.pdf
```

または：

```bash
single-docs build ./docs --format both -o ./dist/
```
...
```bash
npm install -g @your-org/single-docs
```

一時実行：

```bash
npx @your-org/single-docs build ./docs -o ./dist/manual.html
```

プロジェクトローカル導入：

```bash
npm install -D @your-org/single-docs
```
...
docker run --rm \
  -v "$PWD:/work" \
  single-docs/single-docs build /work/docs -o /work/dist/manual.html
```

### 8.4 GitHub Actions

```yaml
- uses: your-org/single-docs-action@v1
  with:
    input: docs
...
```text
single-docs-windows-x64.exe
single-docs-linux-x64
single-docs-macos-x64
single-docs-macos-arm64
```
...
初期は `single-docs.config.yml` を標準とする。
...
<body>
  ...
  <script>
    window.__SINGLE_DOCS_DATA__ = {{ siteDataJson }};
  </script>
...
```text
Single Docs: Init
Single Docs: Build HTML
Single Docs: Build PDF
Single Docs: Preview
Single Docs: Watch Preview
Single Docs: Validate Links
```
...
```json
{
  "singleDocs.configFile": "single-docs.config.yml",
  "singleDocs.outputFile": "dist/manual.html",
  "singleDocs.preview.autoRefresh": true
}
```
...
```bash
single-docs build tests/fixtures/mixed-basic/docs -o tmp/manual.html
```
...
* `single-docs build ./docs -o ./dist/manual.html` が動作する
...
* `single-docs build ./docs --format pdf -o ./dist/manual.pdf` が動作する
...
`single-docs` は、複数ファイルで管理されたドキュメントを、配布しやすい単一ファイルに変換するためのツールである。
