...
リンク切れ・画像欠落・タイトル欠落・見出しレベルの飛び・`alt` 属性の無い画像を、**出力を書き出さずに** 検出します。CI 向けで、エラーがあれば非ゼロ終了します。`--strict` を付けると警告でも失敗します。
...
終了コードは報告が公開する `severity` に従います。これが、リリースで検査を足しても、まだ誰も読んでいない所見のために緑のジョブが赤くならない理由です。警告をリリースのゲートにしたい場合は、ワークフローで `--strict` と 1 度書いてください。Mermaid はブラウザなしで検証するため、pre-render の実描画や図の構文エラーはここでは検査されません。

`validate` はビルドと同じパイプラインを走らせます。したがって `validate` が報告する検査は、すべてビルドも報告します（ビルドは同じ警告を出したうえで、出力を書き出します）。逆は成り立ちません。PDF を書き出すビルドは、その作業でしか分からないこと——ページ番号の帯に対して下余白が足りない、このマシンのどのフォントも描けない文字がある——も報告します。
