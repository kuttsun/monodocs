...
すべて任意で、いずれも monodocs が解釈しない文字列です。文字列に対して唯一行うのは前後の空白の
除去で、空白だけの値は未設定として扱います。届く先は 3 か所です。
...
hash route は読者がコピーするリンクです。単一ファイルとして渡り歩く文書では、どこを見ればよいかを
人が人に伝える唯一の手段なので、チャットのログ、チケット、別の文書へと残ります。そしてページの名前を
変えると、そのすべてが黙って壊れます。リンクをたどった読者は、正常に見える文書の、違うページに着きます。
...
別名とは、今はこのページに解決される古い route のことです。どのページにも当たらない hash が来ると、
文書は対応表を引き、hash を現在の route に置き換えてページを表示します。したがってアドレスバーには、
次にコピーしても働くリンクが残ります。アンカー付きの route（`#/setup/install#configuration`）は、
置き換えを越えてアンカーを保ちます。アンカーが指しているのはパスではなく見出しだからです。

規則は、読者が発見するのではなくビルド時に検査されます。

- 別名は実 route を**すべて試したあと**に照合されるので、ページを隠すことはできません。ある別名が
  主張している route にページが現れた場合はページが勝ち、別名は黙って優先されるのではなく、隠された
  ことを警告します。
- **2 つのページが同じ別名を主張するのはエラー**です。どちらかが走査順で勝つことになり、それは書き手が
  推論できるものではありません。
...
- 別名はサイドバーにも検索インデックスにも前後ナビにも現れません。ページではなく、ページが応答する
  名前だからです。`hidden` なページも別名を保ちます。誰かが既に持っているリンクは、ナビゲーションでは
  ないからです。

別名は自動生成しません。monodocs はリポジトリの履歴を読んで、あるファイルが持っていたすべての route を
記録できますが、そうすると文書の対応表がどのクローンでビルドしたかに依存します——CI の浅いチェックアウトは
完全なクローンとは違うファイルを作ります。別名は、書き手が書いた 1 行です。
