...
Markdown の生 HTML はこれまでどおり破棄します。monodocs はこの 2 つの厳密な綴りだけを認識し、
自分で組み立てた要素に置き換えるので、書いた属性が出力へ届くことはありません。それ以外を含む
マーカー——2 つ目の属性、余分なクラス、タグの間の文字——は、修復されるのではなく他の生 HTML と
同じく破棄されます。

改ページが改ページであることから、2 つのことが導かれます。

- **マーカーはそれ自体が 1 つのブロックであること。** 引用・リスト項目・表のセル・見出しの中に
  置いても何も起きません。それらは印刷時に分割しないブロックだからです。
