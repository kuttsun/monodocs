...
- `style` 綴りではコロンの後の空白は省略でき、末尾の `;` も許されます。
  `style="page-break-after:always;"` も同じマーカーです。
- ASCII の空白・タブ・改行は、`<div` の後、`=` の前後、`>` の前、そしてマーカー全体の前後で
  許されます。`>` と `</div>` の**間**には空白 1 つも許されません。
- それ以外はすべて破棄します。`<DIV>`、`class="page-break foo"`、2 つ目の属性、自己終了の
  `<div class="page-break"/>`、`style` の中の他の宣言。
