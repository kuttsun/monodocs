...
文書のどこからでも `/` または `Ctrl+K`（macOS では `⌘K`）でサイドバーの検索ボックスへ移動できます。読んでいる箇所を離れて検索欄まで辿り着く必要はありません。サイドバーを閉じていても使えます（先に開きます）。どちらのキーも、入力欄で文字を打っている間は横取りしません。`/` はそのまま打てますし、`Ctrl+K` は macOS での編集操作（行末まで削除）のまま残ります。`⌘K` だけは例外で、入力欄の中でも効き、入力済みの語を選択した状態にするので、そのまま打てば前の検索語を打ち替えられます。

サイドバーの検索ボックスから、タイトル・見出し・本文を検索できます。キーワードを空白（全角空白も可）で区切ると、すべての語を含むページだけに絞り込めます。結果は語の出現箇所（タイトル → 見出し → 本文）で順位付けされ、見出しに一致した結果はその見出しへ直接移動します。照合は大文字小文字を区別せず、全角英数字（`ＰＤＦ`）は半角と同じに扱い、カタカナとひらがなも同じものとして扱い（`インストール` で `いんすとーる` が引けます）、長音記号とダッシュ（`ー` / `―`）、波ダッシュと全角チルダ（`〜` / `～`）の書き分けも区別しません。部分一致で行うため日本語のキーワードに分かち書きは不要です。半角カタカナと送り仮名の揺れ（`引き渡し` / `引渡し`）は吸収しません。

結果はマウス無しでも扱えます。`↓` / `↑` で辿り（端では反対側へ回り込みます）、`Enter` で選択中の結果を（まだ動かしていなければ先頭の結果を）開き、`Escape` で検索欄を消します。その間カーソルは検索欄に残るので、そのまま入力を続けて絞り込めます。

結果を開くと、着いたページの本文でもキーワードが強調されます。強調はそのまま別のページへ移っても続くので、ブラウザの検索で探し直さなくても一致箇所が分かります。クエリを打ち替えるか消すと強調は外れます。
